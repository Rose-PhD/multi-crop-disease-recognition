...
# evaluation
def evaluate(model, model_path, val_loader, device, fold_dir, crops, global_labels, global_to_crop_dis, crop_to_global_ids):
    ...
    plot_cm(
        cm_dis, 
        global_labels, fold_dir / "cm_disease_pred_crop.png",  
        "Disease Confusion Matrix (Pred Joint Class)"
    )
    pd.DataFrame(
        cm_dis, 
        index=global_labels, columns=global_labels).to_csv(
        fold_dir / "cm_disease_pred_crop.csv"
    )

    cm_dis_oracle = confusion_matrix(
        true_global, pred_global_true_crop,
        labels=range(len(global_labels)))
    plot_cm(
        cm_dis_oracle, global_labels, fold_dir / "cm_disease_true_crop.png",
        "Disease Confusion Matrix (Oracle Crop Mask)"
        )
    pd.DataFrame(
        cm_dis_oracle, 
        index=global_labels, 
        columns=global_labels).to_csv(fold_dir / "cm_disease_true_crop.csv")
...
# training
def train_fold(fold, model, train_loader, val_loader, device, fold_dir, crops, global_labels, global_to_crop_dis, crop_to_global_ids):
    ...
    for epoch in range(1, 50 + 1):
        ...
        logger.info("Fold %d | Epoch %3d | Train: %.4f | Val: %.4f", fold, epoch, total_train, total_val)
...
# per-config CV loop
def run_config( name, unfreeze_from, config_dir, items, global_index, global_labels, global_to_crop_dis, crop_to_global_ids, crops, num_joint_classes,
