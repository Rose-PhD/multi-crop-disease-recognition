...
Two-stage inference: predicted crop slice and oracle (true) crop slice.
"""
...
from configs.paths import (
    HIER_SAVE_ROOT as MODEL_BASE,
    HIER_TEST_ROOT as TEST_ROOT,
    HIER_TEST_SAVE_ROOT as SAVE_ROOT,
...
logger = logging.getLogger("hier.test")


def discover_configs(model_base):
    """Read config.json from every variant sub-folder written by train_hier.py."""
    configs = []
...
def evaluate_region_fold(model, model_path, loader, device, fold_dir, crops, global_labels):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_crop_idx     = out_crop.argmax(1)

            for i in range(imgs.size(0)):
                ci_true = int(yc[i].item())
                ci_pred = int(pred_crop_idx[i].item())
                ...
                if gi_true >= 0:
                    ...
                    gi_pred_pred   = s_pred + int(out_dis[i, s_pred:e_pred].argmax().item())

                    s_true, e_true = model.crop_slices[ci_true]
                    gi_pred_true   = s_true + int(out_dis[i, s_true:e_true].argmax().item())

                    true_global_all.append(gi_true)
                    pred_global_pred_crop_all.append(gi_pred_pred)
                    pred_global_true_crop_all.append(gi_pred_true)
...
def run_variant(name, unfreeze_from, model_root, crops, diseases_by_crop,
                global_index, global_labels, region_dirs, tform, device, variant_save):
...
def main():
    ...
    setup_logger("hier.test", save_root / "test.log")
    logger.info("Starting hierarchical testing — save root: %s", save_root)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info("Device: %s", device)

    crops, diseases_by_crop, global_index, global_labels, _, _ = load_label_maps(MODEL_BASE)
